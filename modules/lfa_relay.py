...
def signal_handler(signal, frame):
  call(["ifconfig", bridgeName, "down"])
  call(["brctl","delbr", bridgeName])
...
def run(params):
  ...
  try:
    if(relayMethod == "bridge"):
      ...
      call( "echo 16384 > /sys/class/net/"+bridgeName+"/bridge/group_fwd_mask" ,shell=True) #Prevent silent dropping on LLDP frames
      
      sdnpwn.message("Bridge setup complete!", sdnpwn.SUCCESS)
      ...
      sdnpwn.message("Bridge & Tunnel setup complete!", sdnpwn.SUCCESS)
